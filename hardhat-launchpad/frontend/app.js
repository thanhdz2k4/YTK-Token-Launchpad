...
let showSendFormBtn, sendModal, sendTokensBtn, sendToAddressInput, sendAmountInput, sendErrorTextSendToken, sendSuccessText, changeAdminBtn;
...
function setupEventListeners() {
    ...
    changeAdminBtn.addEventListener('click', changeAdminBtnClicked);
...
async function changeAdminBtnClicked() {
    try {
        const ownerAddress = await launchpadContract.owner();
        if(ownerAddress == userAddress) {
            // User is the admin, show admin controls
             window.location.href = 'admin.html';
        } else {
            showAlert('You are not the admin', 'danger');
        }
    } catch (error) {
        console.error('Error getting owner:', error);
        showAlert('Error getting owner address');
    }
}
